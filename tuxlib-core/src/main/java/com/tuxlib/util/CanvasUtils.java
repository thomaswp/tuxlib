...
import com.tuxlib.util.Cache.Key;
...
//		debug("createCircle");
	...
	public static CanvasImage createText(String text,
		...
		return createText(text, new TextFormat(graphics().createFont(
				fontName, Style.PLAIN, fontSize), true), color, 0, 0);
	}
	
	public static CanvasImage createText(String text,
			TextFormat format, int color) {
		return createText(text, format, color, 0, 0);
	}
	
	/** Creates an {@link ImageLayer} of the given text */
	public static CanvasImage createText(String text,
			TextFormat format, int color, float strokeWidth, int strokeColor) {
		if (text.isEmpty() || text == null) {
			text = " ";
		}
//		debug("createText");
		TextLayout layout = PlayN.graphics().layoutText(text, format);
		CanvasImage image = PlayN.graphics().createImage(layout.width(), layout.height() * 1.05f);
